...
import { TCategory } from "@/types/category";
import { useAddProductMutation } from "@/redux/features/product/product";
import { CloudinaryImageRef, CreateProductPayload } from "@/types/product";
import { toast } from "sonner";
...
export function AddProductModal({
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      // Step 1–3: upload cover, variant, and bulk images directly to Cloudinary
      const uploaded = await uploadAll({
        coverImage: data.coverImage.file,
        variantImages: productImages.map((img) => img.file as File),
        bulkImages,
      });

      toast.loading("Creating product...", { id: toastId });

      // Step 4: build JSON payload — no binary data, ever
      const payload: CreateProductPayload = {
        ...
        gender: data.gender,
        coverImage: {
          publicId: uploaded.coverImage.publicId,
          imageUrl: uploaded.coverImage.secureUrl,
        },
        productImages: uploaded.variantImages.map(
          (img: CloudinaryUploadResult, index: number) => ({
            publicId: img.publicId,
            imageUrl: img.secureUrl,
            color: productImages[index].color,
            stock: productImages[index].stock,
            isActive: productImages[index].isActive,
            isPrimary: productImages[index].isPrimary,
          }),
        ),
        bulkImages: uploaded.bulkImages.map((img: CloudinaryUploadResult) => ({
          publicId: img.publicId,
          imageUrl: img.secureUrl,
        })),
      ...
      try {
        ...
        // Product creation failed AFTER images were already uploaded —
        // clean up the now-orphaned Cloudinary assets.
        await rollbackUploads();
        toast.error(createError?.data?.message || "Failed creating product!", {
          id: toastId,
          duration: 2000,
        });
      }
                              ...
                              <Input
                                ...
                                onChange={(e) => {
                                  const value =
                                    Number.parseInt(e.target.value) || 0;
                                  field.onChange(value);
                                  updateProductImage(index, "stock", value);
                                }}
                                className={
                                  errors.productImages?.[index]?.stock
                                    ? "border-red-500"
